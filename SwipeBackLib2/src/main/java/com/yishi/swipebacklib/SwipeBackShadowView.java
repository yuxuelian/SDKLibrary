...
class SwipeBackShadowView extends FrameLayout {
    ...
    void unBindPreActivity(boolean isNeedAddImageView) {
        ...
        if (mPreDecorView != null && mPreContentView != null) {
            ...
            if (!(mPreContentView instanceof SwipeBackLayout)) {
                ...
                int height = mPreDecorView.getHeight() - SystemUiUtils.getNavigationBarHeight(activity);
                if (!SystemUiUtils.isPortrait(activity)) {
                    width = mPreDecorView.getWidth() - SystemUiUtils.getNavigationBarHeight(activity);
    ...
    private Bitmap getBitmap(View view) {
        ...
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache(),
                0,
                0,
                SystemUiUtils.getRealScreenWidth(mActivity),
                SystemUiUtils.getRealScreenHeight(mActivity) - SystemUiUtils.getNavigationBarHeight(mActivity));
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            ...
            if (preActivity != null) {
                ...
                decorView.setTranslationX(-decorView.getMeasuredWidth() * (1 - WE_CHAT_STYLE_MAX_OFFSET) * (1 - slideOffset));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void onPanelClosed(Activity currentActivity) {
        try {
            Activity preActivity = SwipeBackManager.getInstance().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
                preActivity.getWindow().getDecorView().setTranslationX(0);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
