 ...
 */
class RCHelper {
    float[] radii = new float[8];   // top-left, top-right, bottom-right, bottom-left
    Path mClipPath;                 // 剪裁区域路径
    boolean mRoundAsCircle = false; // 圆形
    private int mDefaultStrokeColor;        // 默认描边颜色
    private Paint mPaint;                   // 画笔
    int mStrokeColor;               // 描边颜色
    private ColorStateList mStrokeColorStateList;// 描边颜色的状态
    int mStrokeWidth;               // 描边半径
    boolean mClipBackground;        // 是否剪裁背景
    Region mAreaRegion;             // 内容区域
    RectF mLayer;                   // 画布图层大小
    ...
    void drawableStateChanged(View view) {
